...
//	int sumWeight;
	...
	public void init (LevelSettings settings)
	{
		_settings = settings;
//		sumWeight = 0;
//		foreach (FallingPrefs fP in _settings.terrains.list) {
//			sumWeight += fP.weight;
//		}
		x = 5f;
		isWorking = true;
	...
	void createTerrain ()
	{
//		GameObject go = Global.instance.pool.get (_settings.terrains.list[Random.Range(0, _settings.terrains.list.Length)], transform.position);
		...
		if (_settings.hasGoose && Random.value < _settings.gooseChance) {
			createBalloon (goosePrefab, false);
		}
	}

	void createBalloon (GameObject prefab, bool randomArea)
	{
		GameObject go = Global.instance.pool.get (prefab, transform.position);
		go.transform.parent = world;
		Balloon tP = go.GetComponent<Balloon> ();
		tP.init ();
